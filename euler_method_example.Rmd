...
Last week you coded and solved a simple two state differential equation.  This week will we work on formalizing a format that I want you to use throughout the course.  I want you to use the format because: 

1.  It requires you to be explicit about what are the states, fluxes, parameters, and initial conditions in the model that you are building.
...
3.  It allows you to use an R function to solve the differential equations so that you can focus on the scientific part of the model.
...
5.  It returns two vectors.  The first vector `c(dVeg, dSoil)` is required and **must** only be the derivatives, and **must** be the in same order as the variables in the `states` vector.  Later this first vector will multiplied by "dt" to solve the equations.  The second list `c(VegGrowth, VegMortality, SoilDecomposition)` is optional and represents anything other than a state variable that you want to save from the time-step for later analysis.  In this case, the second vector includes the fluxes.
...
I recommend including comments that define the units of the parameters so that you explicitly state the model currency (g C), model spatial scale (m2), and model temporal scale (yr).  All state variables and parameters should be on the same currency, spatial scale, and temporal scale.  
...
Again the comment with the units are not required but highly encouraged.
...
5. Use the Euler method, also called forward differencing, to solve the differential equations.  This is the same method that we used in Lab 3.
...
solved_vegetation_model <- function(x, npp, mortalityRate){
  ...
  t <- x
  #This is your solution for c by
